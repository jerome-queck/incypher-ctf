...
| The Solver | The agent's own code — the seams it reaches the outside world through, the Instance an Attempt deploys to reach a Target at all, the deterministic cascade every Attempt opens with, and the record it leaves behind: every Step, every Observation, and the credentials taken out of both | `solver/`, starting at `board.py` |
