...
what decides whether it may spend a submission slot. Five strengths, and the ordering between them
...
(an Observation did carry it, and then the Instance that minted it expired underneath it), and
**stated** (the Board's own prose carried it and the model repeated it — the only strength there is
evidence *against*, and so the only one held back from a Board that states unlimited attempts until
the reserved tail, ADR-0021). A candidate is not a Flag until the Board says so; keeping the two
words apart is what stops "we found the Flag" from meaning five different things.
_Avoid_: found flag, the flag (before a verdict), guess — *guessed* is one of the five strengths
