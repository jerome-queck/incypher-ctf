...
def _as_record(sighting: Sighting) -> dict[str, Any]:
    ...
    return {
        ...
        # written down for the same reason: every one of these is a `ctfd-chall-manager` field, so
        # the only Board in reach that can populate them is the scored one, unattended, once
        # (`solver/instance.py`). `timeout` is renewability — `check_source_can_patch_instance` is
        # `if not challenge.timeout` — so without it a Run where nothing was ever renewable reads
        # exactly like a Run whose renewal never fired.
