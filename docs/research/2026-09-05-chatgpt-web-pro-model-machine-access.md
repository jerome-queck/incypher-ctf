...
| **Workspace Agents API** | A genuine official API for triggering a published workspace agent, but not a result-bearing inference API; policy risk is low only as documented. [Workspace Agents][workspace-agents] | Requires an admin-enabled, scoped access token stored as a secret and rotated/revoked through its documented lifecycle. It can enqueue unattended work, but is not a personal Pro credential. [Workspace Agent auth][workspace-agent-auth] | It durably queues text input, accepts an idempotency key and exposes status across caller restarts. It has no file, tool or model-selection field; no retrievable output/extraction path; and no published context/output, concurrency/rate or latency limit. Quota coupling is undocumented. Admin disablement, token revocation or lost agent access stops new calls. [Workspace Agents][workspace-agents] | Cannot request or verify a Pro model and cannot return the answer to the Solver. **Rejected.** |
