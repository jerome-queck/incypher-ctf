...
SCRIPT = Path(__file__).resolve().parent.parent / "scripts" / "apply_tool_supply_modes.py"
...
def test_git_transport_modes_are_restored_from_the_validated_inventory(tmp_path: Path) -> None:
    ...
    result = subprocess.run(
        [sys.executable, str(SCRIPT), "--inventory", str(inventory), "--root", str(root)],
        capture_output=True,
        text=True,
    )
...
def test_a_destination_outside_the_image_namespace_is_refused(tmp_path: Path) -> None:
    ...
    result = subprocess.run(
        [sys.executable, str(SCRIPT), "--inventory", str(inventory), "--root", str(tmp_path)],
        capture_output=True,
        text=True,
    )
