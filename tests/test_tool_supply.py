...
def test_duplicate_component_ids_across_profile_locks_fail_before_assembly(tmp_path: Path):
    ...
    duplicate["profile_id"] = "second-profile"
    second_fixture = source / "fixtures" / "second-profile" / "identity.txt"
    second_fixture.parent.mkdir(parents=True)
    second_fixture.write_text("tool-supply-probe\n")
    duplicate["components"][0]["files"][0]["source"] = "fixtures/second-profile/identity.txt"
...
def test_two_components_cannot_claim_one_image_destination(tmp_path: Path):
    ...
    second["profile_id"] = "second-profile"
    second["components"][0]["component_id"] = "fixture.second"
    second_fixture = source / "fixtures" / "second-profile" / "identity.txt"
    second_fixture.parent.mkdir(parents=True)
    second_fixture.write_text("tool-supply-probe\n")
    second["components"][0]["files"][0]["source"] = "fixtures/second-profile/identity.txt"
    (source / "locks" / "second.json").write_text(json.dumps(second) + "\n")

    result = assemble(source, tmp_path / "assembled")

    assert result.returncode != 0
    assert "duplicate image destination: /opt/solver/tool-supply/identity.txt" in result.stderr
...
def test_package_and_file_inventory_order_is_canonical(tmp_path: Path):
    ...
    assert [declared["destination"] for declared in component["files"]] == [
