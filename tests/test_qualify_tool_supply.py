...
def test_native_docker_observation_cleans_its_host_cgroup_directly() -> None:
    ...
    assert runner.commands[0] == ["sudo", "mkdir", "/sys/fs/cgroup/incypher-v2-strict.slice"]
    assert runner.commands[1][runner.commands[1].index("--cgroup-parent") + 1] == "incypher-v2-strict.slice"
    probe = runner.commands[2]
